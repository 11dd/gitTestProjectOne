...
- (void) setObj:(id)aa ImageArray:(NSMutableArray *)array
{
    ...
    //[self.MyImageView sd_setImageWithURL:[NSURL URLWithString:self.imgOb.url]];
...
-(void)singleTap:(UITapGestureRecognizer*)recognizer{//参数轻敲类型的
    ...
    self.img_Scroll.backgroundColor = [UIColor clearColor];
    for (int i = 0; i< self.arr_Imgs.count; i++) {
        ...
        UIScrollView * scro = [[UIScrollView alloc]initWithFrame:CGRectMake(i*BWidth, -20, BWidth, BHeight-20)];
        ...
        scro.tag = i+1;
        scro.contentSize =  CGSizeMake(BWidth, BHeight-20);
        UIImageView *imgv = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, (CGFloat)w, (CGFloat)h)];
        imgv.tag = 1000 + i ;
        imgv.center = CGPointMake(BWidth/2, BHeight/2-20);
        
        imgv.image = [UIImage imageNamed:jj.name];
        
        //[imgv sd_setImageWithURL:[NSURL URLWithString:jj.url]];
...
//    [self.window addSubview:self.windowView];
    ...
    [self addSubview:self.img_Scroll];
...
- (UIView *) topView
{
    if (!_topView) {
        ...
        [btn_back setImage:[UIImage imageNamed:@"leftaaa"] forState:UIControlStateNormal];
